...
class Controller{
  ...
  async _deleteData(tableName, id) {
	...
	updateData(tableName) {
		return async (req, res, next) => {
			try {
				let updated = await this._updateData(tableName, req.params.id, req.body);
  ...
  createStandardControllers(tableName) {
    return {
        ...
        updateData: { method: 'put', main: this.updateData(tableName), options: {} },
        deleteData: { method: 'delete', main: this.deleteData(tableName), options: {} }
      }
  }
}
